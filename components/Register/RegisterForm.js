...
const RegisterForm = () => {
    ...
    const style ={
        ...
        btn:'transition duration-[300ms] bg-opacity-[0.8] hover:bg-opacity-[0.95] my-6 bg-red-400 mx-auto px-8 md:px-12 py-2 md:py-3 text-stone-800 font-bold tracking-wide',
        }
  return (
    <form className={style.wrapper} onSubmit={submitHandler}>
        <input type="text" className={style.input} placeholder="Your Name here..." />
        <input type="email" className={style.input} placeholder="Your Email here..." />
        <input type="text" className={style.input} placeholder="Your Phone here..." />
        <input type="password" className={style.input} placeholder="Password " />
        <button className={style.btn} style={{ fontFamily: 'Poppins, sans-serif' }} >Register</button>
    </form>
  )
}
